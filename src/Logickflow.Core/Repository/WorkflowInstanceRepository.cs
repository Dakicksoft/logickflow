using System;
using System.Linq;
using Logickflow.Core.Data;
using Logickflow.Core.Data.Entity;
using Logickflow.Core.Data.Mapper;
...
namespace Logickflow.Core.Repository
{
    internal class WorkflowInstanceRepository
    {
        ...
        public void Save(IWorkflowInstance workflowInstance)
        {
            using (var dbContext = DbFactory.GetDbContext())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new ApplicationException("Data operation is abnormal, the transaction has been rolled back", ex);
                }
            }
        }
        ...
        private void Insert(IWorkflowInstance workflowInstance, DbContext dbContext)
        {
            ...
            //The newly created workflow instance does not have previous nodes that need to be persisted
        ...
        private IActivityTemplate FindNextActivityTemplate(IActivityTemplate template)
        {
            return
                template.AllowedActions.First(
                    p => p.OperationCode == OperationCode.Submit).Transit;
        }
